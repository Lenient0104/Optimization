...
class Optimization:
    ...
    def map_energy_to_lanes(self, user_weight, user_pal, edges, flag):
        for connection_id, edge_data in tqdm(edges.items(), desc="Processing edges"):
            from_speed = edges[connection_id]['from_speed']
            to_speed = edges[connection_id]['to_speed']
            edges[connection_id]['from_ebike_energy'] = self.ebike_energymodel.calculate(from_speed, user_weight, user_pal)
            edges[connection_id]['to_ebike_energy'] = self.ebike_energymodel.calculate(to_speed, user_weight, user_pal)
            ...
            edges[connection_id]['from_escooter_energy'] = self.escooter_energymodel.calculate(from_speed, user_weight, 0)
