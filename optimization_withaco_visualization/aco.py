...
class Ant:
    ...
    def calculate_move_probabilities(self, next_moves, change_device_probability, walking_preference):
        ...
        for next_move in next_moves:
            ...
            if key in edges:
                ...
                # change_mode = 1
                #
                # # Check if the ant will change the device probabilistically if random.random() <
                # return_device_probability and current_mode != mode:  # current = scooter, mode = scooter # we
                # consider change the device, while next possible mode is different # If changing device,
                # consider station availability and energy sufficiency if mode == 'e-scooter' and not
                # self.has_scooter_station(next_edge): change_mode = 0 elif mode == 'e-bike' and not
                # self.has_bike_station(next_edge): change_mode = 0 elif mode == 'e-scooter' and not
                # self.has_bike_station(next_edge): change_mode = 0 else: # The modes are the same, so no change
                # should occur change_mode = 0
                #
    ...
    # def choose_available_mode(self, edge_id):
    #
    #     return mode
