...
class TestDQN(unittest.TestCase):
    ...
    def test_run_dqn(self):
        ...
        with open('results/Q_learning_station-new.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Experiment ID', 'Episode', 'Simulation Time', 'Station Number', 'Travel Time Cost (seconds)',
            ...
            for station in self.station_num:
                ...
                for test_index in range(test_size): # 500 od pairs
                    ...
                    optimizer = Optimization(self.net_xml_path, self.user, self.db_path, self.simulation_time[0], station, source_edge,
                                             target_edge)
