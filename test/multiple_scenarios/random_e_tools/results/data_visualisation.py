...
def plot_box_plot_travel_time(filtered_data):
    ...
    unique_ants = filtered_data['E-tools'].unique()

    # Prepare data for plotting
    # all_data = [filtered_data[filtered_data['Number of Ants'] == ant]['Travel Time Cost (seconds)'].dropna() for ant in
    #             unique_ants]
    all_data = [filtered_data[filtered_data['E-tools'] == ant]['Travel Time Cost (seconds)'].dropna() for ant in
                unique_ants]

    # Plot box plot
    plt.figure(figsize=(12, 8))
    plt.boxplot(all_data, labels=unique_ants)
    plt.xlabel('Distribution of E-mobility Tools')
    plt.ylabel('Travel Time Cost (seconds)')
    plt.title('ACO Box Plot of Travel Time Cost for Different Distribution of E-mobility Tools')
...
def plot_box_plot_execution_time(filtered_data):
    ...
    # all_data = [filtered_data[filtered_data['Number of Ants'] == ant]['Travel Time Cost (seconds)'].dropna() for ant in
    #             unique_ants]
    all_data = [filtered_data[filtered_data['E-tools'] == ant]['Execution Time (seconds)'].dropna() for ant in
                unique_ants]
    ...
    plt.title('ACO Box Plot of Execution Time for Different Distribution of E-mobility Tools')
    # plt.autoscale()
...
# Call the functions
filtered_data = filter_data("Updated_ACO-random.csv")
