...
def plot_box_plot_travel_time(filtered_data):
    ...
    unique_ants = filtered_data['User Preference'].unique()

    # Prepare data for plotting
    # all_data = [filtered_data[filtered_data['Number of Ants'] == ant]['Travel Time Cost (seconds)'].dropna() for ant in
    #             unique_ants]
    all_data = [filtered_data[filtered_data['User Preference'] == ant]['Travel Time Cost (seconds)'].dropna() for ant in
                unique_ants]
    ...
    plt.title('ACO Box Plot of Travel Time Cost for Different User Preferences')
...
def plot_box_plot_execution_time(filtered_data):
    ...
    # all_data = [filtered_data[filtered_data['Number of Ants'] == ant]['Travel Time Cost (seconds)'].dropna() for ant in
    #             unique_ants]
    all_data = [filtered_data[filtered_data['User Preference'] == ant]['Execution Time (seconds)'].dropna() for ant in
                unique_ants]
    ...
    plt.title('ACO Box Plot of Execution Time for Different User Preferences')
    plt.savefig('ACO Box Plot of Execution Time')
    plt.show()


# Call the functions
filtered_data = filter_data("Updated_ACO_1.csv")
