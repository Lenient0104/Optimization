...
class TestDQN(unittest.TestCase):
    ...
    def test_run_dqn(self):
        ...
        with open('results/Q-learning-withoutecar.csv', 'w', newline='') as file:
            ...
            for test_index in range(test_size): # 500 od pairs
                ...
                if graph is None:
                    writer.writerow([test_size + 1, self.episodes[0], self.simulation_time[0], self.station_num[0], self.energy_rate[0], 0, 0, None, False])
                    continue
                best_route, best_modes, total_time_cost, execution_time, find = Q_learning_agent.run_q_learning(
                    optimizer, source_edge, target_edge, self.episodes[0], self.energy_rate[0])
                ...
                writer.writerow([experiment_id, self.episodes[0], self.simulation_time[0], self.station_num[0], self.energy_rate[0], total_time_cost, execution_time,best_modes, best_route, find])
