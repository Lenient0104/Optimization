...
class TestDQN(unittest.TestCase):
    ...
    def test_run_dqn(self):
        ...
        optimizer = Optimization(self.net_xml_path, self.user, self.db_path, self.simulation_time[0], 10, self.start_edge,
                                 self.destination_edge)
        graph = optimizer.new_graph
        if graph is None:
            print('invalid graph')
        else:
            best_route, best_modes, total_time_cost, execution_time, find = DQN.run_dqn(optimizer, self.start_edge,
                                                                                         self.destination_edge, self.episodes[0], 1)
            print(best_route)
            print(best_modes)
