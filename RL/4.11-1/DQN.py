...
class Environment:
    def __init__(self, graph, source, destination, initial_energy=10000):
        self.graph = graph
        self.source = source
    ...
    def step(self, action_index):
        ...
        if next_node in self.visited_nodes:
            # print("current node", env.current_node, "next node", next_node)
            reward = -10000000000
        ...
        # Calculate energy consumption for new mode
        ...
        # Not enough energy for current mode
        if mode == self.last_mode and self.remaining_energy - energy_consumed_current < 0:
            # print("no enough energy for current mode, we stop this time")
        ...
        # Not enough energy for new mode
        if mode != self.last_mode and new_energy - energy_consumed_new < 0:
            # print("no enough energy for new mode")
            # Action not feasible due to energy constraint, so don't change mode
            info = {'current_node': self.current_node, 'mode': self.last_mode, 'action_taken': 'Insufficient energy '
                                                                                               'for new mode'}
            return self._get_state(), -100000, False, info  # Now includes info

        # Enough energy for current mode
        if mode == self.last_mode and self.remaining_energy - energy_consumed_current >= 0:
            # print("enough energy for current mode, we dont change the mode")
        ...
        # Enough energy for new mode
        elif mode != self.last_mode and new_energy - energy_consumed_new >= 0:
            # print("mode changed")
            # print(time_cost)
            self.remaining_energy = new_energy - energy_consumed_new  # refill
...
class DQNAgent:
    def __init__(self, state_dim, action_dim, hidden_dim=128, lr=0.001, gamma=1, epsilon=1, epsilon_decay=0.005,
                 min_epsilon=0.01, buffer_size=1000000, batch_size=64):
...
def infer_best_route(agent, env, max_steps=1000):
    ...
    while steps < max_steps:
        ...
        if num_available_actions == 0:
            print("No actions")
            break
        # print("visited nodes", env.visited_nodes)
        action = agent.act(state, num_available_actions, test=True)
        # print("Action index: ", action)
        print("next edge:", possible_actions[action][0])
        # print("next edge-3:", possible_actions[action+2][0])
        print("current mode", env.last_mode)
        print("remaining energy:", env.remaining_energy)
        next_state, reward, done, info = env.step(action)

        print("------------------------------")
        print("reward:", reward)
        print("new mode: ", info['mode'])
        print("new edge:", info['current_node'])
        print("remaining energy", env.remaining_energy)
...
for episode_count in episodes:
    ...
    for test_index in range(test_size):  # Run 30 times for each episode count
        ...
        for episode in range(episode_count):
            ...
            states = set()
            states.add(tuple(state))  # Convert numpy array to tuple before adding to the set
            ...
            while not done:
                ...
                if tuple(next_state) in states:  # Convert to tuple before checking
                    break
                state = next_state
                states.add(tuple(state))  # Convert to tuple before adding to the set
