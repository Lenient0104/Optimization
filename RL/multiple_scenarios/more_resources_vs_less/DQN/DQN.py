...
with open('results/DQN_experiment_results_simulation', 'w', newline='') as file:
    ...
    for simulation in simulation_time:
        ...
        for test_index in range(test_size):
            ...
            action_dim = max(
                len(list(graph.out_edges(node))) for node in
                graph.nodes)  # Max number of possible actions from any node
            env = Environment(graph, source_edge, target_edge)
            ...
            for episode in range(episodes[0]):
                total_reward = 0
                state = env.reset()
            ...
            experiment_id = f"{episodes[0]}-{test_index}"
            writer.writerow([experiment_id, episodes[0], simulation, execution_time, total_time_cost, find])
