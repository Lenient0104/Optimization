...
class Optimization:
    ...
    def build_graph(self):
        ...
        congested_edges = {
            '3789252#2', '3789252#3', '401912432#4', '16247623',
            '361409608#1', '37865098#2', '3789572', '3791905#0',
            '43538577#3', '4396059#1', '4408238#0'
        }
        ...
        for connection in self.connections:
            ...
            to_edge = connection[1]
            length = self.edge_map[from_edge][
                'length']  # Assume the length from the first edge is used for the whole connection

            # Determine the speed based on whether the edge is special or not
            if from_edge in congested_edges or to_edge in congested_edges:
                # Set all mode speeds to 0.1 m/s for special connections
                travel_times = {
                    'e_bike_1': length / 0.5,
                    'e_car': length / 0.1,
                    'e_scooter_1': length / 0.5,
                    'walking': length / 1.5
                }
            else:
                # Normal speed values for non-special connections
                travel_times = {
            ...
            G.add_edge(from_edge, to_edge, travel_times=travel_times, distance=length)
