...
class Ant:
    ...
    def move(self):
        ...
        if move_probabilities is None:
            ...
            if len(self.path) > 1:
                ...
                self.decrease_pheromones(previous_edge, current_location, previous_step[1])  # Decrease pheromones for the edge leading to the dead end
            return
        ...
        if self.graph.has_edge(current_edge, previous_edge):
            for edge_data in self.graph.edges[current_edge, previous_edge]:
                if edge_data['mode'] == mode:
                    ...
                    self.graph.edges[current_edge, previous_edge][edge_data]['pheronmone_level'] = (1 - evaporation_rate) * penalty_factor
    ...
    def get_best_vehicle(self, mode):
        ...
        if mode == 'e_bike_1':
            e_bike_id = 'e_bike ' + str(random.randint(0, 10))
            soc = random.randint(70, 100)
            return e_bike_id, soc
        elif mode == 'e_scooter_1':
            e_scooter_id = 'e_scooter ' + str(random.randint(0, 10))
            soc = random.randint(70, 100)
            return e_scooter_id, soc
        elif mode == 'e_car':
            e_car_id = 'e_car ' + str(random.randint(0, 10))
            soc = random.randint(70, 100)
            return e_car_id, soc
