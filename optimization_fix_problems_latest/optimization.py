...
class Optimization:
    def __init__(self, net_xml_path, edges_csv_path, user):
        self.user = user
        ...
        self.map_energy_to_lanes(user.weight, user.pal, self.edges)
    ...
    def map_speeds_to_lanes(self, filename, edges):
        ...
        for connection_id, edge_data in tqdm(edges.items(), desc="Processing edges"):
            ...
            # edges[connection_id]['from_speed'] = from_speed
            # edges[connection_id]['to_speed'] = to_speed
    ...
    def map_energy_to_lanes(self, user_weight, user_pal, edges):
        for connection_id, edge_data in tqdm(edges.items(), desc="Processing edges"):
            from_speed = edges[connection_id]['from_speed']
            to_speed = edges[connection_id]['to_speed']
            edges[connection_id]['from_ebike_energy'] = self.ebike_energymodel.calculate(from_speed, user_weight, 0,
                                                                                         user_pal)
            edges[connection_id]['to_ebike_energy'] = self.ebike_energymodel.calculate(to_speed, user_weight, 0, user_pal)
            ...
            edges[connection_id]['from_escooter_energy'] = self.escooter_energymodel.calculate(from_speed, user_weight,
                                                                                               0)
            edges[connection_id]['to_escooter_energy'] = self.escooter_energymodel.calculate(to_speed, user_weight, 0)

    def convert_data_structure(self, edges):
        new_structure = {}

        for connection_id, edge_info in edges.items():
            # Extract 'from' and 'to' station names
            from_station = edge_info['from_edge']
            to_station = edge_info['to_edge']
            ...
            for mode in edge_info['mode_time']:
                ...
                # Initialize the station in the new structure if not already present
                if from_station not in new_structure:
                    new_structure[from_station] = {}

                # Initialize the mode of transport for the station if not already present
                if mode not in new_structure[from_station]:
                    new_structure[from_station][mode] = []

                # Append the new tuple
                new_structure[from_station][mode].append((to_station, mode, total_energy, total_time))

        return new_structure

        # The new_structure now holds the transformed data
    ...
    def build_graph_from_edges(self, edges):
        ...
        print(self.convert_data_structure(edges))
