...
class Optimization:
    ...
    def parse_net_xml(self, filepath):
        ...
        for conn in tqdm(root.findall('connection'), desc="Processing connections"):
            ...
            edges[connection_id] = {
                ...
                'speed_limit': 13.9,
                'mode_time': {
                    'e_bike_1': {'from_time': 0.0, 'to_time': 0.0},
                    'e_car': {'from_time': 0.0, 'to_time': 0.0},
                    'e_scooter_1': {'from_time': 0.0, 'to_time': 0.0},
                    'walking': {'from_time': 0.0, 'to_time': 0.0}
                },
                'speed': {
    ...
    def map_time_to_gausian(self):
        stddev_coefficient = 0.1

        # 更新每个 edge 的 mode_time
        for edge_id, edge_data in self.edges.items():
            for mode, times in edge_data['mode_time'].items():
                ...
                # 更新 from_time 和 to_time
                edge_data['mode_time'][mode]['from_time'] = max(0, random.gauss(average_time, stddev))
                edge_data['mode_time'][mode]['to_time'] = max(0, random.gauss(average_time, stddev))
    ...
    def find_optimal_and_suboptimal_path(self, ants):
        """
        :param ants:
        :return: percentile_80, percentile_80_subopt, ants_at_percentile_80
        """
        filtered_rate = 0.8
        optimal_ant = None
        cost_time = [ant.total_time_cost for ant in ants]
        # Calculate the 80th percentile
        percentile_80 = np.percentile(cost_time, filtered_rate * 100)
        # Find ant(s) whose total_time_cost is the 80th percentile
        ants_at_percentile_80 = [ant for ant in ants if ant.total_time_cost == percentile_80]
        # Filter the data to get values below or equal to the 80th percentile
        filtered_data = [x for x in cost_time if x <= percentile_80]
        # Now, to filter out these values from the original data
        remaining_data = [x for x in cost_time if x not in filtered_data]
        # Calculate the 80th percentile
        percentile_80_subopt = np.percentile(remaining_data, filtered_rate * 100)
        return percentile_80, percentile_80_subopt, ants_at_percentile_80
    ...
    def run_aco_algorithm(self, start_edge, destination_edge, number_of_ants, number_of_iterations, mode='walking'):
        ...
        for ant_num in range(number_of_ants):
            ant = Ant(start_edge, destination_edge, self.db_connection, self.edges, ant_index, mode)
            # print("ant", ant_index)
            move_count = 0
